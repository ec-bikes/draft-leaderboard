// This project uses a patched version of spacetime which only supports UTC.
import spacetime, {
  type ParsableDate,
  type Spacetime,
 ...
 * The ones returned by this project's date utils will always use UTC time.
 */
...
/**
 * Get a spacetime object in UTC time from a date string or other value.
 * Throws if the value is invalid.
 */
export function parseDate(value: ParsableDate, options?: SpacetimeConstructorOptions): SafeDate {
  const date = spacetime(value, 'UTC', options);
  if (!date.isValid()) {
...
/**
 * Format a date as a string in UTC. (Use `date.unixFmt()` for custom formats.)
 *
 * - `isoDate`: YYYY-MM-DD
 * - `shortDate`: e.g. "22 Mar 2024"
 * - `datetime`: e.g. "19 Mar 2024, 02:01 GMT"
 */
export function formatDate(date: SafeDate, format: KnownFormatNames): string {
...
export function now(): SafeDate {
  return spacetime.now('UTC');
}

/** Get the date/time for the beginning of today (UTC) */
export function today(): SafeDate {
  return spacetime.today('UTC');
}
